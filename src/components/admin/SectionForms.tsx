    ...
    >
      ...
      <label>
        ...
        <textarea
          rows={4}
          {...register("more")}
          placeholder='Optional second paragraph, e.g. Our approach is rooted in "Cyber-Industrialism"...'
        />
      </label>
  ...
  return {
    ...
    pillars:
      rawPillars.length > 0
        ...
        : [
            { icon: "security", title: "CYBER SECURITY", description: "" },
            { icon: "online", title: "DATA & CLOUD", description: "" },
            { icon: "innovation", title: "SOFTWARE & DEV", description: "" },
            { icon: "corporate", title: "CONSULTING", description: "" },
          ],
  };
}
...
function toAboutHeroDefaultValues(
  ...
  return {
    ...
    stat1Value: Array.isArray(data.stats)
      ? String((data.stats as Array<Record<string, unknown>>)[0]?.value ?? "")
      : "14+",
    stat1Label: Array.isArray(data.stats)
      ? String((data.stats as Array<Record<string, unknown>>)[0]?.label ?? "")
      : "GLOBAL NODES",
    stat2Value: Array.isArray(data.stats)
      ? String((data.stats as Array<Record<string, unknown>>)[1]?.value ?? "")
      : "99.99%",
    stat2Label: Array.isArray(data.stats)
      ? String((data.stats as Array<Record<string, unknown>>)[1]?.label ?? "")
      : "PROTOCOL UPTIME",
  };
}
